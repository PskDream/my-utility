import type { TripActivityImage, TripDay, TripHighlights, TripMeta } from '@/types/trip'
import { wikimediaFilePath, wikimediaFilePageUrl } from '@/utils/maps'

function wikimediaImage(fileName: string, alt: string): TripActivityImage {
  return {
    url: wikimediaFilePath(fileName),
    alt,
    creditLabel: 'Wikimedia Commons',
    creditUrl: wikimediaFilePageUrl(fileName),
  }
}

export const tripMeta: TripMeta = {
  title: 'แพลนทริป Kyushu',
  dateRange: '19–25 พ.ย. 2569 (7 วัน 6 คืน)',
  routeSummary: 'Fukuoka → Yufuin → Beppu → Aso → Kumamoto → Fukuoka (+ Itoshima day trip)',
  stats: ['7 คน', 'รถเช่า 2 คัน'],
  concept:
    'Scenic > Walking > Food > Park > Cafe > Local (Market = ลูกเล่นย่อยของ Food + Local) — เน้น 60% Experience/Scenic/Food/Local, 40% Landmark ไม่ใช่ทริปที่ต้องไปให้ครบ แต่เป็นทริปที่กลับมาแล้วรู้สึกว่าได้สัมผัส Kyushu จริง ๆ',
  priorityWeights: [
    { category: 'scenic', weight: 25 },
    { category: 'walking', weight: 20 },
    { category: 'food', weight: 20 },
    { category: 'park', weight: 15 },
    { category: 'cafe', weight: 10 },
    { category: 'local', weight: 10 },
  ],
  budgetNote:
    'งบอาหารโดยประมาณ ¥4,000–8,000 / คน / วัน (breakfast ¥500–1,000 · lunch ¥1,000–3,000 · street food/cafe ¥500–1,500 · dinner ¥2,000–5,000) — รวม 7 คน ~¥28,000–56,000/วัน วันที่ Aso/Itoshima/Fukuoka อาจสูงกว่านี้เล็กน้อย',
}

export const tripHighlights: TripHighlights = {
  top: [
    'Daikanbo — Aso Scenic Drive',
    'Lake Kinrin',
    'Sakurai Futamigaura — Itoshima Coast',
    'Hyotan Onsen',
    'Jigoku Mushi — Jigokumushikobo Kannawa (Beppu)',
    'Akaushi Lunch — Aka Gyu-don Iwasaki (Aso)',
    'Yanagibashi Rengo Market — Food Walk (Fukuoka)',
    'Motsunabe — Tokunagaya (Fukuoka)',
    'เดินเล่น Kego / Daimyo / Imaizumi (Fukuoka)',
    'Suizenji Joju-en / Ezu Lake Park',
  ],
  secondary: [
    'Kusasenri',
    'Kannawa',
    'Toriten — Wataru / Toyoken (Beppu)',
    'Kumamoto Ramen — Kokutei Honten',
    'Hakata Ramen — Shin Shin (Tenjin)',
    'Yusentei Park',
    'Ohori Park',
    'Nakasu Yatai',
    'Sagiridai',
    'Beppu Jigoku Meguri',
  ],
  optional: [
    'Aso Volcano Museum',
    'Kumamoto Castle',
    'Basashi — Agura / Oniku no Taisho (Kumamoto, ไม่ใช่ทุกคนกินม้าได้)',
    'Nagahama Fish Market / Uoza (เช็ค Citizen\'s Appreciation Day ปี 2026 ก่อน)',
    'เดินเล่นย่าน Yoshizuka + Little Asia Market (คืนที่กลับที่พักเร็ว)',
    'Canal City',
    'LaLaport',
    'Donki',
    'Fukuoka Castle Ruins',
  ],
}

export const tripDays: TripDay[] = [
  {
    day: 1,
    weekday: 'พฤหัส',
    date: '19 พ.ย.',
    route: 'Fukuoka → Yufuin → Beppu',
    theme: 'Soft Landing — เริ่มทริปแบบ Scenic Countryside',
    feel: '🌿 Yufuin scenery · 🚶 เดินเล่น · 🍜 มื้อแรกใน Kyushu · ☕ คาเฟ่ · 🚗 Scenic drive · 🏨 Beppu',
    driveInfo: 'Fukuoka→Yufuin ~2 ชม. (Oita Expressway) · Yufuin→Beppu ~45 นาที (Yamanami Highway)',
    activities: [
      { time: '07:35', activity: 'ถึงสนามบิน Fukuoka (International Terminal)', mapQuery: 'Fukuoka Airport International Terminal' },
      { time: '07:35–08:20', activity: 'ตม. + รับกระเป๋า', note: '7 คนเผื่อเวลาไว้ 45 นาที' },
      { time: '08:20–09:00', activity: 'รับรถเช่า 2 คัน', note: 'เช็ครอบรถ ตั้ง ETC ตั้ง Google Maps' },
      { time: '09:00–09:30', activity: 'Lawson ซื้ออาหารเช้า + น้ำ' },
      { time: '09:30', activity: 'ออกเดินทางไป Yufuin' },
      {
        time: '11:30–12:00',
        activity: 'ถึง Yufuin Station — เริ่มเดิน',
        note: 'ไม่ต้องรีบเอารถไปจอดหน้าทุกจุด เริ่มจากเดินจาก Station เข้าเมือง',
        categories: ['walking'],
        mapQuery: 'Yufuin Station',
      },
      {
        time: '12:00–13:00',
        activity: 'Lunch — Yufu Mabushi "Shin" (Yufuin Ekimae)',
        note: 'เมนู mabushi เฉพาะถิ่น · ~¥3,000–4,000/คน · Tabelog ~3.50 อยู่ห่าง Yufuin Station เดิน 1 นาที ร้านเล็ก 7 คนควรโทรจองล่วงหน้า ถ้าคิวยาวเกิน 20 นาทีเปลี่ยนไปสาขา Kinrinko (ใกล้ Lake Kinrin แต่ไม่มี parking ของตัวเอง)',
        categories: ['food'],
        priority: 'secondary',
        mapQuery: 'Yufu Mabushi Shin Yufuin Ekimae',
      },
      {
        time: '13:00–14:00',
        activity: 'เดินเล่น Yufuin (Station → ซอยเล็ก → ร้านขนม → คาเฟ่)',
        note: 'ผสม Tourist + Local ไม่ใช่เดินตรงไป shopping street อย่างเดียว',
        categories: ['walking', 'local'],
        priority: 'top',
      },
      {
        time: '14:00–14:45',
        activity: 'Yunotsubo Kaido',
        note: 'จำกัดเวลา 45 นาที เน้น "เห็นอะไรน่ากินก็ซื้อ" ไม่ต้องซื้อของฝากให้ครบ',
        categories: ['walking', 'local'],
        mapQuery: 'Yunotsubo Kaido Yufuin',
      },
      {
        time: '14:45–15:30',
        activity: 'Lake Kinrin — เดินรอบทะเลสาบ',
        note: 'ใบไม้เปลี่ยนสีเป็น bonus ไม่การันตี peak ช่วงนี้',
        categories: ['scenic', 'walking'],
        priority: 'top',
        mapQuery: 'Lake Kinrin Yufuin',
        image: wikimediaImage('Lake Kinrin in Yufuin, Oita - Aug 24, 2018 (1).jpg', 'Lake Kinrin ใน Yufuin'),
      },
      {
        time: '15:30–16:10',
        activity: 'คาเฟ่ริมทะเลสาบ Kinrin',
        note: 'เลือกจากวิว + ที่นั่ง 7 คน มากกว่าชื่อดัง',
        categories: ['cafe'],
        priority: 'secondary',
      },
      {
        time: '16:10–16:40',
        activity: 'Sagiridai Overlook',
        note: 'ชมวิว Yufuin Basin จากมุมสูง 20-30 นาทีพอ',
        categories: ['scenic'],
        priority: 'secondary',
        mapQuery: 'Sagiridai Observatory Yufuin',
      },
      { time: '17:00', activity: 'ออกเดินทาง Yufuin → Beppu', note: 'ทาง Yamanami Highway' },
      {
        time: '17:45',
        activity: 'Check-in ที่พัก Beppu',
        note: '6-3 Noguchi Motomachi, Beppu',
        mapQuery: '6-3 Noguchi Motomachi Beppu',
      },
      {
        time: '19:00',
        activity: 'Dinner — Toriten (ไก่ทอดสไตล์ Beppu)',
        note: '~¥1,000–3,000/คน · ตัวเต็ง Toriten Shokunin Wataru (ใกล้ Beppu Station, รับจอง) หรือ Restaurant Toyoken (ร้านต้นกำเนิด toriten, จอด ~60 คัน, dinner course รับจองกลุ่ม 4+ คน แต่ lunch ไม่รับจอง)',
        categories: ['food'],
        priority: 'secondary',
        mapQuery: 'Toriten Shokunin Wataru Beppu',
      },
      { time: '21:00', activity: 'เดินเล่นย่านสถานี Beppu ตอนกลางคืน (ถ้ายังไม่เหนื่อย)', categories: ['local'] },
    ],
    callout: 'วันแรกไม่ต้องเร่ง — ถ้าถึง Yufuin ช้ากว่าแผน ให้ตัด Yunotsubo หรือ Sagiridai ก่อน แล้วเก็บ Kinrin Lake ไว้',
  },
  {
    day: 2,
    weekday: 'ศุกร์',
    date: '20 พ.ย.',
    route: 'Beppu',
    theme: 'Local Onsen Town — ไม่ใช่ไปดูบ่อน้ำร้อน 7 บ่อ แต่มาสัมผัสเมือง',
    feel: '🏘️ ชีวิต Beppu · 🌋 Geothermal · 🍜 ของกิน local · 🚠 ดูวิว · ♨️ ออนเซ็น',
    driveInfo: 'Beppu → Kannawa ~15-20 นาที',
    activities: [
      { time: '09:00–10:00', activity: 'ตื่นสาย + breakfast/coffee ใกล้ที่พัก', categories: ['cafe'] },
      { time: '10:00–11:00', activity: 'ขับไป Kannawa' },
      {
        time: '11:00–12:30',
        activity: 'เดินเล่น Kannawa',
        note: 'ไอน้ำ ถนนเนิน โรงอาบน้ำ steam cooking — ย่านที่เห็นเอกลักษณ์ Beppu ชัดที่สุด',
        categories: ['local', 'walking'],
        priority: 'secondary',
        mapQuery: 'Kannawa Onsen Beppu',
      },
      {
        time: '12:30–13:30',
        activity: 'Lunch — Jigoku Mushi ที่ Jigokumushikobo Kannawa',
        note: '~¥1,000–2,000/คน · Tabelog ~3.9 เกือบ 4,000 รีวิว · เลือกวัตถุดิบเอง (เนื้อ/ปลา/ผัก) แล้วนึ่งด้วยไอน้ำพุร้อน',
        categories: ['food', 'local'],
        priority: 'top',
        mapQuery: 'Jigoku Mushi Kobo Kannawa',
      },
      {
        time: '13:30–15:00',
        activity: 'Beppu Jigoku Meguri (เลือกเอง 2-3 บ่อ)',
        note: 'Quality > Quantity ไม่ต้องครบ 7 บ่อ ถ้าเริ่มเบื่อตัดได้เลย',
        categories: ['scenic'],
        priority: 'secondary',
        mapQuery: 'Umi Jigoku Beppu',
      },
      {
        time: '15:00–15:40',
        activity: 'Yukemuri View — เมืองมองจากมุมสูง',
        categories: ['scenic'],
        priority: 'secondary',
      },
      {
        time: '15:40–16:30',
        activity: 'Beppu Ropeway (ถ้าอากาศดี)',
        note: 'เที่ยวขึ้นสุดท้าย 16:30 น. / ลงสุดท้าย 17:00 น. (winter schedule) — ถ้าถึงช้ากว่านี้หรืออากาศไม่ดี ให้ตัดทันที อย่าเสี่ยงตกรอบ',
        categories: ['scenic'],
        priority: 'secondary',
        alert: true,
        mapQuery: 'Beppu Ropeway',
      },
      {
        time: '17:30–20:00',
        activity: '⭐ Hyotan Onsen',
        note: 'เปิดถึง 01:00 (large bath ถึงเที่ยงคืน) ไม่ต้องรีบ — Large Bath / Outdoor Bath / Steam Bath / Sand Bath',
        categories: ['onsen'],
        priority: 'top',
        mapQuery: 'Hyotan Onsen Beppu',
        image: wikimediaImage('Hyotan Onsen (5011201676).jpg', 'ทางเข้า Hyotan Onsen ที่ Beppu'),
      },
      {
        time: '20:00–21:00',
        activity: 'Dinner (Jigoku Mushi / ของกิน local ถ้ายังไม่ได้กินที่ Hyotan)',
        categories: ['food'],
      },
    ],
  },
  {
    day: 3,
    weekday: 'เสาร์',
    date: '21 พ.ย.',
    route: 'Beppu → Aso → Kumamoto',
    theme: 'Scenic Road Trip — ถนนคือ Attraction',
    feel: '🚗 Road trip · 🌿 ภูเขา · 🍜 Akaushi · 🚶 Aso · 🌆 Kumamoto Night',
    driveInfo:
      'ขับรวม ~190 กม. / 3.5+ ชม. · เติมน้ำมันก่อนขึ้นเขา · จอดรถ 2 คันเผื่อเวลามากกว่าคันเดียว เผื่อ +15-20 นาทีต่อจุด',
    activities: [
      { time: '08:30–09:30', activity: 'Slow morning — breakfast + เดินเล่นใกล้ที่พัก' },
      {
        time: '09:30',
        activity: 'ออกเดินทาง Beppu → Aso (Yamanami Highway)',
        note: 'เจอวิวสวยจอดได้เลย ไม่ต้องปักหมุดทุกจุด',
      },
      {
        time: '11:30–12:15',
        activity: 'Daikanbo — วิว Aso Caldera',
        note: 'อากาศเย็น เตรียมแจ็คเก็ต',
        categories: ['scenic'],
        priority: 'top',
        mapQuery: 'Daikanbo Aso',
        image: wikimediaImage(
          'A view of the five mountains of Aso from Daikanbo viewpoint.jpg',
          'วิวห้ายอดเขา Aso จาก Daikanbo',
        ),
      },
      {
        time: '12:15–13:00',
        activity: 'Scenic drive ผ่าน Aso landscape',
        note: 'ถ้าเส้นทางผ่าน Ubuyama พอดี แวะ UBUYAMA PLACE ได้ (wood-fired pizza + วิวทุ่ง, Tabelog 3.58) — เป็น scenic food ระหว่างทาง ไม่ใช่จุดหมาย ไม่ควรขับอ้อมเพื่อมัน',
        categories: ['scenic'],
      },
      {
        time: '13:00–14:00',
        activity: 'Lunch — Akaushi Beef ที่ Aka Gyu-don Iwasaki',
        note: '~¥1,000–2,000/คน · Tabelog ~3.60/360+ รีวิว ราคาไม่แรงเหมาะกับกลุ่ม 7 คน · สำรอง: Nagomi Shokudo (Tabelog ~3.20 แต่มี Akaushi + Aso Milk Pork ราคา ~¥2,000–3,000) ถ้าคิวยาวเกินไป · ควรโทรจอง/เช็คคิวล่วงหน้าสำหรับ 7 คน',
        categories: ['food'],
        priority: 'top',
        mapQuery: 'Aka Gyu-don Iwasaki Aso',
      },
      {
        time: '14:15–15:15',
        activity: 'Kusasenri — ทุ่งหญ้า',
        categories: ['scenic', 'walking'],
        priority: 'secondary',
        mapQuery: 'Kusasenri Aso',
      },
      {
        time: '15:15–15:45',
        activity: 'Aso Volcano Museum (ถ้าฝนตก/หนาวมาก/สนใจ)',
        note: 'ถ้าอากาศดี เลือกวิวก่อน ไม่ต้องเข้า',
        priority: 'optional',
        mapQuery: 'Aso Volcano Museum',
      },
      { time: '15:45–17:30', activity: 'ขับ Aso → Kumamoto' },
      {
        time: '18:00',
        activity: 'Check-in Kumamoto',
        note: 'Kasuga 1-chome, 5-3',
        mapQuery: 'Kasuga 1-chome Chuo-ku Kumamoto',
      },
      {
        time: '19:00',
        activity: 'Dinner — Kumamoto Ramen ที่ Kokutei Honten',
        note: '< ¥1,000/คน · Tabelog ~3.68/2,500+ รีวิว ติด Tabelog Ramen 100 (West) 2025 — คืนนี้ไม่ยัด Castle ขอรู้จัก Kumamoto ก่อน',
        categories: ['food', 'local'],
        priority: 'top',
        mapQuery: 'Kumamoto Ramen Kokutei Honten',
      },
      {
        time: '20:30',
        activity: 'Basashi (ถ้ามีคนในกลุ่มอยากลอง) — Agura หรือ Oniku no Taisho',
        note: 'เนื้อม้าดิบสไตล์ Kumamoto ไม่ใช่ทุกคนกินได้ ไม่บังคับ',
        categories: ['food', 'local'],
        priority: 'optional',
        mapQuery: 'Kumamoto Basashi Agura',
      },
    ],
    callout:
      'ถ้ารถติดจน Daikanbo ล่ากว่า 12:00 ให้ตัด Kusasenri ก่อน แล้วรักษา Akaushi lunch กับเวลาไป Kumamoto ให้ทัน check-in',
  },
  {
    day: 4,
    weekday: 'อาทิตย์',
    date: '22 พ.ย.',
    route: 'Kumamoto → Fukuoka',
    theme: 'History + Garden + City Transfer',
    feel: '🏯 Castle เช้า · 🌳 Garden · 🌿 Lake (+ Hanahaku 2026) · 🚗 Fukuoka',
    driveInfo: 'Kumamoto → Fukuoka ~110 กม. / 2.5 ชม.',
    activities: [
      {
        time: '08:30–10:30',
        activity: 'Kumamoto Castle',
        note: 'ดูตอนเช้าแทนตอนเย็น ไม่ต้องพยายามดูทุกห้อง 1.5-2 ชม.พอ',
        priority: 'secondary',
        mapQuery: 'Kumamoto Castle',
      },
      {
        time: '10:45–12:00',
        activity: 'Suizenji Joju-en',
        categories: ['park', 'scenic', 'walking'],
        priority: 'top',
        mapQuery: 'Suizenji Jojuen Kumamoto',
        image: wikimediaImage('Kumamoto Suizenji-jojuen01n4272.jpg', 'สวน Suizenji Joju-en ที่ Kumamoto'),
      },
      {
        time: '12:00–13:30',
        activity: 'Ezu Lake / Suizenji Ezu Park',
        note: '⭐ ตรงกับงาน Kumamoto Hanahaku 2026 (20-29 พ.ย.) ที่ใช้ Ezu Lake Park เป็นพื้นที่หลัก — ถ้าเวลาไม่พอ เลือกอย่างใดอย่างหนึ่งระหว่าง Suizenji กับ Ezu Park ให้ยาวขึ้น แทนที่จะรีบทั้งคู่',
        categories: ['park', 'local', 'walking'],
        priority: 'top',
        mapQuery: 'Ezu Lake Kumamoto',
      },
      { time: '13:30–14:30', activity: 'Lunch Kumamoto (local ไม่ต้องหรู)', categories: ['food'] },
      { time: '14:30–17:00', activity: 'ขับ Kumamoto → Fukuoka' },
      {
        time: '17:00–18:00',
        activity: 'Check-in Fukuoka',
        note: 'Yoshizuka, Hakata Ward',
        mapQuery: 'Yoshizuka Hakata Ward Fukuoka',
      },
      {
        time: '19:00',
        activity: 'Hakata Night — Hakata Ramen / Seafood / Yakitori + เดินเล่น Hakata',
        note: 'เก็บ Motsunabe ไว้กินคืน 24 พ.ย. แทน (มีร้านตัวเต็งรอแล้ว)',
        categories: ['food', 'local'],
      },
    ],
    callout:
      'ที่พักอยู่ย่าน Yoshizuka ซึ่งมีความ residential/multicultural กว่า Tenjin — คืนไหนกลับที่พักเร็ว ลองเดินเล่นแถวนี้ + แวะ Little Asia Market แทนการไปห้างอีกรอบ',
  },
  {
    day: 5,
    weekday: 'จันทร์',
    date: '23 พ.ย.',
    route: 'Fukuoka → Itoshima',
    theme: 'Coastal Scenic Drive',
    feel: '🌊 ทะเล · 🌿 Coastal drive · 🍜 Seafood · ☕ คาเฟ่ริมทะเล · 🌅 Sunset',
    driveInfo: 'ขับเลียบชายฝั่ง ไป-กลับ Fukuoka ↔ Itoshima',
    activities: [
      { time: '09:00', activity: 'ออกจาก Fukuoka' },
      {
        time: '10:00–11:00',
        activity: 'Sakurai Futamigaura',
        note: 'เสาโทริอิกลางทะเล + หินคู่ฟูฟุอิวะ',
        categories: ['scenic'],
        priority: 'top',
        mapQuery: 'Sakurai Futamigaura Itoshima',
        image: wikimediaImage('Sakurai Futamigaura.jpg', 'เสาโทริอิกลางทะเลที่ Sakurai Futamigaura'),
      },
      {
        time: '11:00–12:00',
        activity: 'Coastal Drive',
        note: 'แวะ beach / cafe / village ที่เจอระหว่างทางได้เลย ไม่ต้องปักหมุดทุกจุด',
        categories: ['scenic'],
      },
      {
        time: '12:00–13:30',
        activity: 'Itoshima Lunch — Itoshima Seafood Restaurant Futamigaura',
        note: 'อยู่ใกล้ Sakurai Futamigaura แทบไม่เสียเวลาเดินทางเพิ่ม · สำรอง: Itoshima Shokudo Homori (~¥1,000–2,000/คน เหมาะมื้อกลางวันแบบ casual) — ทั้งย่าน Itoshima มีร้านซีฟู้ดเยอะมาก ไม่จำเป็นต้องขับอ้อมไปหาร้านคะแนนสูงกว่านิดหน่อย',
        categories: ['food'],
        priority: 'top',
        mapQuery: 'Itoshima Seafood Restaurant Futamigaura',
      },
      {
        time: '13:30–15:00',
        activity: 'คาเฟ่ริมทะเล',
        note: 'ปล่อยชีวิต ไม่ต้องรีบ',
        categories: ['cafe'],
        priority: 'top',
      },
      { time: '15:00–16:00', activity: 'เดินชายหาด', categories: ['walking', 'scenic'] },
      {
        time: '16:00–17:00',
        activity: 'Sunset',
        note: 'Main Event ของวัน ถ้าอากาศดี',
        categories: ['scenic'],
        priority: 'top',
      },
      { time: '17:00–18:30', activity: 'ขับกลับ Fukuoka' },
      {
        time: '19:00',
        activity: 'Dinner Fukuoka — Motsunabe / Yakitori / Izakaya',
        note: 'จัดหนักได้คืนนี้',
        categories: ['food'],
      },
    ],
    callout: 'วันจันทร์คาเฟ่บางร้านในอิโตชิมะอาจปิด — เช็คก่อนไปถึง ถ้าปิดให้สลับไปคาเฟ่ตัวสำรองระหว่าง Coastal Drive',
  },
  {
    day: 6,
    weekday: 'อังคาร',
    date: '24 พ.ย.',
    route: 'Fukuoka Food & Local Day',
    theme: 'City Life — วันเดินเมือง + ตลาด ไม่ใช้รถก็ได้',
    feel: '🐟 Yanagibashi Market · 🍜 Ramen · 🌳 Ohori Park · 🚶 Kego/Daimyo/Imaizumi · 🌳 Yusentei · 🍲 Motsunabe · 🏮 Yatai',
    activities: [
      {
        time: '08:00–09:30',
        activity: '🐟 Yanagibashi Rengo Market — Food Walk',
        note: '"ครัวของ Hakata" ตลาดสด + ร้านอาหารทะเล/ของแห้ง มีคนท้องถิ่นมาซื้อของจริง ไม่ต้องตั้งใจกินมื้อใหญ่ เดินชิมไปเรื่อย ๆ (ซาชิมิ ของกินเล่น ข้าว/ของท้องถิ่น เครื่องดื่ม) — ตลาดเปิด ~08:00–18:00 แต่ร้านแต่ละร้านเวลาไม่เท่ากัน มาเช้าจะเห็นบรรยากาศดีที่สุด',
        categories: ['food', 'local'],
        priority: 'top',
        mapQuery: 'Yanagibashi Rengo Market Fukuoka',
        image: wikimediaImage('Yanagibashi Rengo Ichiba by LWY in Fukuoka.jpg', 'บรรยากาศตลาด Yanagibashi Rengo'),
      },
      {
        time: '09:30–10:30',
        activity: 'เดินจาก Yanagibashi → Haruyoshi → Kego → Daimyo',
        note: 'เปลี่ยนจากตลาดสดเข้าสู่ย่านเมืองแบบ natural',
        categories: ['walking', 'local'],
      },
      { time: '10:30–11:30', activity: 'คาเฟ่', categories: ['cafe'] },
      {
        time: '12:00–14:00',
        activity: 'Ohori Park + Japanese Garden',
        note: 'เดินรอบทะเลสาบ → Japanese Garden ไม่ต้องเข้า Museum',
        categories: ['park', 'walking', 'scenic'],
        priority: 'top',
        mapQuery: 'Ohori Park Fukuoka',
      },
      {
        time: '14:00–15:00',
        activity: 'Lunch — Hakata Ramen ที่ Shin Shin (Tenjin)',
        note: '~¥1,000–2,000/คน · รีวิวเยอะมาก (Tenjin) — ชื่อดังมากกว่าคะแนน Tabelog สูงสุดในหมวดนี้ ถ้าคิวยาวเปลี่ยนร้านราเมงอื่นแถว Tenjin ได้',
        categories: ['food'],
        priority: 'top',
        mapQuery: 'Shin Shin Ramen Tenjin Fukuoka',
      },
      {
        time: '15:00–16:00',
        activity: 'เดินต่อ Daimyo → Imaizumi → ร้านเล็ก ๆ',
        note: 'ดูคน Fukuoka ใช้ชีวิต ไม่ใช่ไปห้าง',
        categories: ['walking', 'local'],
        priority: 'top',
        mapQuery: 'Daimyo Fukuoka',
        image: wikimediaImage(
          'Kon-ya-machi Yokochō the ENE view from about the middle Daimyō 1-chōme Chūō-ku Fukuoka 20250605.jpg',
          'ซอยเล็กย่าน Daimyō, Fukuoka',
        ),
      },
      {
        time: '16:00–17:30',
        activity: 'Yusentei Park',
        note: 'Japanese garden + เมเปิล ~150 ต้น ช่วง autumn foliage',
        categories: ['park', 'scenic'],
        priority: 'secondary',
        mapQuery: 'Yusentei Park Fukuoka',
      },
      {
        time: '18:00–19:30',
        activity: 'Dinner — Motsunabe ที่ Hakata Motsunabe Tokunagaya Souhonten',
        note: '~¥4,000–5,000/คน · Tabelog ~3.73/1,000+ รีวิว — สำรอง (route-first): Motsunabe Rakutenchi สาขา Imaizumi ถ้าอยากต่อจากการเดิน Kego→Daimyo→Imaizumi เลยโดยไม่ต้องย้ายโซน',
        categories: ['food'],
        priority: 'top',
        mapQuery: 'Hakata Motsunabe Tokunagaya Souhonten',
      },
      {
        time: '20:00',
        activity: 'Nakasu Yatai (ของว่างรอบสอง ไม่ใช่มื้อใหญ่)',
        note: 'กินมาเยอะแล้ว ให้เป็น dessert/experience: ไม้ปิ้ง 1-2 ไม้ แชร์ราเมง เกี๊ยวซ่า เครื่องดื่ม — เดินหลายร้าน ไม่นั่งร้านเดียว 2 ชม. ร้านเล็ก 7 คนอาจต้องแยก 2 กลุ่ม',
        categories: ['food', 'local'],
        priority: 'top',
        mapQuery: 'Nakasu Yatai Fukuoka',
      },
      {
        time: 'ช่วงว่าง',
        activity: 'Shopping — Donki / Tenjin / Hakata Station / Canal City',
        note: 'Free Time หลังอาหารเย็นหรือช่วงว่าง ไม่ใช่ Main Plan',
        priority: 'optional',
      },
    ],
    callout: 'คืนนี้ห้ามไปไกล Fukuoka และไม่ควรไป Itoshima ตอนค่ำ เพราะพรุ่งนี้ต้องออกเดินทางไปสนามบินตั้งแต่เช้ามืด',
  },
  {
    day: 7,
    weekday: 'พุธ',
    date: '25 พ.ย.',
    route: 'Fukuoka → สนามบิน → บินกลับ',
    theme: 'เดินทางกลับ',
    activities: [
      { time: '05:30', activity: 'ตื่น เตรียมตัว' },
      { time: '06:00', activity: 'Check-out' },
      {
        time: '06:15',
        activity: 'Taxi ไปสนามบิน',
        note: 'ต้องจองคืนก่อน 7 คน + กระเป๋า = 3 คัน',
        alert: true,
        mapQuery: 'Fukuoka Airport',
      },
      { time: '07:00–07:30', activity: 'Check-in + โหลดกระเป๋า' },
      { time: '07:30–08:00', activity: 'ตม. ขาออก' },
      { time: '09:30', activity: 'บินกลับ' },
    ],
  },
]
